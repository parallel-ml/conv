...
def filter():
    ...
    residual = Conv2D(128, (1, 1), strides=(2, 2), padding='same', use_bias=False)(x)
    x = separable.filter(x, 128, (3, 3), padding='same')
    x = Activation('relu')(x)
    x = separable.filter(x, 128, (3, 3), padding='same')
    x = MaxPooling2D((3, 3), strides=(2, 2), padding='same')(x)
    x = layers.add([x, residual])
    ...
    residual = Conv2D(256, (1, 1), strides=(2, 2), padding='same', use_bias=False)(x)
    x = Activation('relu')(x)
    x = separable.filter(x, 256, (3, 3), padding='same')
    x = Activation('relu')(x)
    x = separable.filter(x, 256, (3, 3), padding='same')
    x = MaxPooling2D((3, 3), strides=(2, 2), padding='same')(x)
    x = layers.add([x, residual])
    ...
    x = separable.filter(x, 728, (3, 3), padding='same')
    x = Activation('relu')(x)
    x = separable.filter(x, 1024, (3, 3), padding='same')
    x = MaxPooling2D((3, 3), strides=(2, 2), padding='same')(x)
    x = layers.add([x, residual])

    x = separable.filter(x, 1536, (3, 3), padding='same')
    x = Activation('relu')(x)
    x = separable.filter(x, 2048, (3, 3), padding='same')
    x = Activation('relu')(x)
