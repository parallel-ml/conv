...
def conv_unit(X, nb_filter, kernal, activation='relu', max_pooling=True, strides=(1, 1)):
    if max_pooling:
...
def xy_unit(X, filters, kernal, max_pooling=True, strides=(1, 1), num=3, padding='valid', activation='relu'):
    ...
    X = conv_xy.conv(X, filters, kernal, strides, 'valid', activation)
    X = conv_xy.merge(X)
    return X


def channel_unit(X, filters, kernal, max_pooling=True, strides=(1, 1), padding='valid', activation='relu'):
    """ Cnn unit with channel separation. """
    if max_pooling:
        X = MaxPooling2D(strides=(2, 2), pool_size=(2, 2))(X)
    X = conv_channel.split(X, 3)
    X = conv_channel.conv(X, filters, kernal, strides, padding, activation)
    X = conv_channel.merge(X)
    return X


def filter_unit(X, filters, kernal, max_pooling=True, strides=(1, 1), padding='valid', activation='relu'):
