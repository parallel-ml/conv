...
def original():
    ...
    x = conv(x, 3, [64, 64, 256], stride=(1, 1))
...
def filter():
    ...
    x = ZeroPadding2D((3, 3))(img_input)
    x = filter_unit(x, 64, (7, 7), stride=(2, 2), activation='relu')
    x = MaxPooling2D((3, 3), strides=(2, 2))(x)
...
def xy():
    ...
    x = ZeroPadding2D((3, 3))(img_input)
    x = xy_unit(x, 64, (7, 7), stride=(2, 2), activation='relu')
    x = MaxPooling2D((3, 3), strides=(2, 2))(x)
...
def channel():
    ...
    x = ZeroPadding2D((3, 3))(img_input)
    x = channel_unit(x, 64, (7, 7), stride=(2, 2), max_pooling=False, padding='valid', activation='relu')
    x = MaxPooling2D((3, 3), strides=(2, 2))(x)

    x = conv(x, (3, 3), [64, 64, 256], stride=(1, 1))
    x = identity(x, (3, 3), [64, 64, 256])
    x = identity(x, (3, 3), [64, 64, 256])

    print Model(img_input, x).summary()
...
def conv(input_tensor, kernel_size, filters, stride=(2, 2)):
    f1, f2, f3 = filters

    if MODE == 'channel':
        x = channel_unit(input_tensor, f1, (1, 1), max_pooling=False, stride=stride, padding='valid')
        ...
        shortcut = channel_unit(input_tensor, f3, (1, 1), max_pooling=False, stride=stride,
                                padding='valid', activation=None)
    elif MODE == 'filter':
        x = filter_unit(input_tensor, f1, (1, 1), max_pooling=False, stride=stride, padding='valid')
        ...
        shortcut = filter_unit(input_tensor, f3, (1, 1), max_pooling=False, stride=stride,
                               padding='valid', activation=None)
    elif MODE == 'xy':
        x = xy_unit(input_tensor, f1, (1, 1), max_pooling=False, stride=stride, padding='valid')
        ...
        shortcut = xy_unit(input_tensor, f3, (1, 1), max_pooling=False, stride=stride,
                           padding='valid', activation=None)
    else:
        x = Conv2D(f1, (1, 1), strides=stride, activation='relu')(input_tensor)
        x = Conv2D(f2, kernel_size, padding='same', activation='relu')(x)
        x = Conv2D(f3, (1, 1))(x)

        shortcut = Conv2D(f3, (1, 1), strides=stride)(input_tensor)
