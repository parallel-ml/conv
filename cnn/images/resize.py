...
def norm_image(img):
    ...
    scale = np.max([np.abs(np.percentile(img_y_np, 1)),
                    np.abs(np.percentile(img_y_np, 99))])
...
def main():
    ...
    test_all = sorted(glob.glob(os.path.join(INPUT_DIR, '*.jpg')), key=natural_key)

    # Make the output directories
    base_out = os.path.join(OUTPUT_DIR)
    test_dir_out = os.path.join(base_out)
    os.makedirs(test_dir_out)

    # Preprocess the training files
    procs = dict()
    procs[1] = Process(target=prep_images, args=(test_all, test_dir_out,))
    procs[1].start()
    procs[1].join()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', dest='input_dir', required=True, help='input directory.')
    parser.add_argument('-o', dest='output_dir', required=True, help='output directory.')
    parser.add_argument('--dim', dest='dim', default=224, type=int, help='image dimension.')
    global args
